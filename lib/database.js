...
const { config, databaseFileLocation, dailyDatabaseFolder } = require('./config')
...
class Database {
  constructor (app) {
    this._app = app
    this._sql = new sqlite.Database(databaseFileLocation)
  }
  ...
  getDailyDatabase () {
    ...
    if (fileName !== this._currentDatabaseCopyFileName) {
      ...
      this._currentDatabaseCopyInstance = new sqlite.Database(join(dailyDatabaseFolder, fileName))
